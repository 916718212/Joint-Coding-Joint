...
class DynamicVFE(nn.Module):
    ...
    def forward(self,
        ...
        h1=calculate_entropy(features)
        n=0
        for i, vfe in enumerate(self.vfe_layers):
            point_feats = vfe(features)
            if i==0:
                h2=calculate_entropy(point_feats)
            if i==1:
                h3=calculate_entropy(point_feats)
            n=n+1
        ...
        path='/root/autodl-tmp/joint-eloss-channel.csv'

        # np.savetxt('/root/autodl-tmp/joint-eloss-channel.txt',eloss,fmt='%.5f',delimiter=',')
